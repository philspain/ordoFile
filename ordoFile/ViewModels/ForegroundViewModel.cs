...
namespace ordoFile.ViewModels
{
    public class ForegroundViewModel : ViewModelBase
    {
        ...
        public string SelectedPreset
        {
            get 
            {
                // If nothing is currently selected, select first item in
                // list of presets
                if (_selectedPreset == null || _selectedPreset == String.Empty)
                {
                    _selectedPreset = _presetNames[0];
                    TempPresetName = _presetNames[0];
                }
        ...
        /// <summary>
        /// Property for getting/setting the TempPresetName string
        /// </summary>
        public string TempPresetName
        {
            get
            {
                return _tempPresetName;
            }
            set
            {
                _tempPresetName = value;
                OnPropertyChanged("TempPresetName");
            }
        }

        /// <summary>
        /// Property for getting/setting the AddTypeText string;
        /// the is the name of a new filetype to be added to an existing
        /// preset filter
        /// </summary>
        ...
        public ObservableCollection<DirectoryModel> RootDirectory
        {
            get 
            {
                ...
                GUIDispatcherUpdates.ClearObservableCollection(_rootDirectoryCollection);
        ...
        public ObservableCollection<string> PresetNames
        {
            get
            {
                ...
                GUIDispatcherUpdates.ClearObservableCollection(_presetNames);
        ...
        public ObservableCollection<string> SelectedPresetTypes
        {
            get
            {
                ...
                GUIDispatcherUpdates.ClearObservableCollection(_selectedPresetTypes);                
        ...
        /// <summary>
        /// Property for getting command used to add/remove type
        /// from a currently selected preset filter depending on
        /// whether or not it's corresponding checkbox is checked.
        /// </summary>
        public ICommand PresetTypeCheckCommand
        {
            get
            {
                if (_presetTypeCheckCommand == null)
                    _presetTypeCheckCommand = new DelegateCommand<CheckBox>(PresetTypeCheck);

                return _presetTypeCheckCommand;
            }
        }

        /// <summary>
        /// Property for getting command used to add a new filtype
        /// to a currently existing preset filter.
        /// </summary>
        public ICommand AddTypeCommand
        {
            get
            {
                if (_addTypeCommand == null)
                    _addTypeCommand = new DelegateCommand(AddType);
        ...
        /// <summary>
        /// Property for getting command used to save any changes
        /// made to existing preset filters.
        /// </summary>
            ...
            if (_subdirCheckboxChecked)
            {
                    ...
                    _fgOrganiser.AvailableFileTypes.Count == 0)
                {
                    ...
                    ThreadPool.QueueUserWorkItem(
                        delegate
                        {
                            GUIDispatcherUpdates.ClearObservableCollection(_fgOrganiser.RootDirectory.Subdirectories);
        ...
        /// <summary>
        /// Add or remove types to/from a preset filter depending
        /// on whether a checkbox is checked or not.
        /// </summary>
        /// <param name="checkBox">Checkbox that represents a filetype.</param>
        void PresetTypeCheck(CheckBox checkBox)
        {
            string type = checkBox.Content as string;

            if ((bool)checkBox.IsChecked)
            {
                if (!_presets.PresetTypeExists(_selectedPreset, type))
                {
                    try
                    {
                        _presets.AddPresetType(_selectedPreset, type);
                    }
                    catch (ArgumentException AEx)
                    {
                        _logger.LogError(AEx.Message + "\n" + AEx.StackTrace);
            ...
            else
            {
                if (_presets.PresetTypeExists(_selectedPreset, type))
                {
                    _presets.RemovePresetType(_selectedPreset, type);
                }
            }
        }

        /// <summary>
        /// Add type to a preset filter.
        /// </summary>
        void AddType()
        {
            if (_presets.PresetExists(_selectedPreset) &&
                !_presets.PresetTypeExists(_selectedPreset, _addTypeText))
            {
                try
                {
                    _presets.AddPresetType(_selectedPreset, _addTypeText);
                }
                catch (ArgumentException AEx)
                {
                    _logger.LogError(AEx.Message + "\n" + AEx.StackTrace);
        ...
        // Save any changes to existing presets.
        void SavePresetEdits()
        {
            try
            {
                _presets.EditPresets(_selectedPreset, _tempPresetName, _presets.GetPresetTypes(_selectedPreset));
            }
            catch (ArgumentException AEx)
            {
                _logger.LogError(AEx.Message + "\n" + AEx.StackTrace);
            ...
            if(_selectedPreset != _tempPresetName)
                SelectedPreset = _tempPresetName;
        }
